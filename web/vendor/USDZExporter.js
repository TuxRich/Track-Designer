// three.js r165 USDZExporter, with three LOCAL PATCHES. If you ever re-download
// this file from the three.js distribution, re-apply them or USDZ export breaks:
//
//  1. Import paths rewritten for this vendor/ folder (fflate + TextureUtils
//     below), since we don't mirror the examples/jsm directory tree.
//  2. Geometry layers are written to the archive root ("Geometry_12.usda")
//     rather than "geometries/Geometry_12.usda"...
//  3. ...and the reference in buildXform() updated to match.
//
// (2) and (3) exist because some USDZ viewers refuse archives that keep USD
// layers in subfolders — nested layers can be used to smuggle references to
// external addresses, so they are rejected outright. Textures may stay in
// textures/, which is why that path is untouched.
		...
		scene.traverseVisible( ( object ) => {
			...
			if ( object.isMesh ) {
				...
				if ( material.isMeshStandardMaterial ) {
					...
					const geometryFileName = 'Geometry_' + geometry.id + '.usda'; // LOCAL PATCH: flat layout
...
function buildXform( object, geometry, material ) {
	...
	prepend references = @./Geometry_${ geometry.id }.usda@</Geometry>
